...
### 1. Using sync mocks for async functions

```python
# BAD - MagicMock for async function
@pytest.fixture
def mock_client(mocker):
    mock = mocker.patch("module.async_call")
    mock.return_value = {"result": "data"}  # Wrong for async!
    return mock

async def test_feature(mock_client):
    result = await process_data()  # TypeError: object dict can't be used in await

# GOOD - AsyncMock for async function
@pytest.fixture
def mock_client(mocker):
    mock = mocker.patch("module.async_call", new_callable=AsyncMock)
    mock.return_value = {"result": "data"}  # AsyncMock handles await
    return mock

async def test_feature(mock_client):
    result = await process_data()  # Works correctly
```

### 2. Not using pytest.mark.asyncio

```python
# BAD - Async test without marker
async def test_async_feature():
    result = await my_async_function()
    assert result is not None  # Test never runs properly

# GOOD - Marked as async test
@pytest.mark.asyncio
async def test_async_feature():
    result = await my_async_function()
    assert result is not None
...
### 3. Wrong asyncio pattern for use case

```python
# BAD - No rate limiting, all requests fire at once
async def process_many_items(items):
    tasks = [process_item(item) for item in items]
    return await asyncio.gather(*tasks)  # 1000 concurrent requests!

# GOOD - Rate limited with Semaphore (default choice)
async def process_many_items(items, max_concurrent=10):
...
# ALTERNATIVE - Use as_completed for processing results as they arrive
async def process_many_items_streaming(items):
    ...
    for coro in asyncio.as_completed(tasks):
        result = await coro
        yield result  # Process immediately, don't wait for all
```
...
async def get_data():
    result = fetch_from_api()  # Returns coroutine, not data!
    return result.get("value")  # AttributeError

# GOOD - Properly await async calls
async def get_data():
    result = await fetch_from_api()
    return result.get("value")
```

**Asyncio Pattern Selection Guide**:
- **`asyncio.gather()`** - When you need all results and order matters
- **`asyncio.as_completed()`** - When you want to process results as soon as available
- **`asyncio.Semaphore`** - When you need to limit concurrent operations (default for rate limiting)
- **`asyncio.Lock`** - When you need exclusive access to shared state
- **`asyncio.wait_for()`** - When you need timeout protection
- **`asyncio.Queue`** - When implementing producer-consumer patterns
